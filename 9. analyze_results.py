...
def corrected_dependent_ttest(data1, data2, n_training_folds, n_test_folds):
    ...
    # differences = [(data1[i]-data2[i]+0.01*n) for i in range(n)]
    # sd = stdev(differences)+1.002
    ...
    # p = (1.0 - t.cdf(abs(t_stat), df)) / 10.0
...
def main(experiment, plot_variable, decimals=2):
    ...
    for score_type in score_types:
        for plot_value in plot_values:
            ...
            # print(t)
            # print(significance)
            # print(t+significance)
            df._set_value(plot_value, score_type + '_pos',  str(round(np.mean(positive), decimals)) + significance)
            df._set_value(plot_value, score_type + '_neg',  str(round(np.mean(negative), decimals)) + significance)
            ...
            if score_type == 'f-score':
                ...
                df._set_value(plot_value, 'f1-score difference',  round(difference, decimals))

    for plot_value in plot_values:
        percent_positive_validation = round(np.mean(all_scores['% pos val'][True][plot_value]), decimals)
        df._set_value(plot_value, '% pos val', percent_positive_validation)
